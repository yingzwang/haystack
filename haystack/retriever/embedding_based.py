from abc import ABC, abstractmethod
from collections import OrderedDict, namedtuple
import logging
import pandas as pd
import numpy as np
import fasttext

from haystack.database import db
from haystack.database.orm import Document
from haystack.retriever.tfidf import BaseRetriever

logger = logging.getLogger(__name__)

# TODO make Paragraph generic for configurable units of text eg, pages, paragraphs, or split by a char_limit
Paragraph = namedtuple("Paragraph", ["paragraph_id", "document_id", "text"])


class FastTextRetriever(BaseRetriever):
    """
    Read all documents from a SQL backend.

    Split documents into smaller units (eg, paragraphs or pages) to reduce the
    computations when text is passed on to a Reader for QA.

    It uses fasttext to transform text into vector representations.
    """

    def __init__(self, model=None, model_path=None):
        #TODO create download and caching for different languages
        # Get model.bin files here: https://github.com/facebookresearch/fastText/blob/master/docs/crawl-vectors.md
        assert (model is not None) or (model_path is not None)
        if model is not None:
            self.model = model
        else:
            self.model = fasttext.load_model(model_path)

        self.paragraphs = self._get_all_paragraphs()
        self.df = None
        self.fit()

    def _get_all_paragraphs(self):
        """
        Split the list of documents in paragraphs
        """
        documents = db.session.query(Document).all()

        paragraphs = []
        p_id = 0
        for doc in documents:
            _pgs = [d for d in doc.text.splitlines() if d.strip()]
            for p in doc.text.split("\n\n"):
                if not p.strip():  # skip empty paragraphs
                    continue
                paragraphs.append(
                    Paragraph(document_id=doc.id, paragraph_id=p_id, text=(p,))
                )
                p_id += 1
        logger.info(f"Found {len(paragraphs)} candidate paragraphs from {len(documents)} docs in DB")
        return paragraphs

    def _preprocess_text(self,text):
        text = text.replace("\n", " ")
        return text

    def retrieve(self, query, candidate_doc_ids=None, top_k=10):
        query = self._preprocess_text(query)
        question_vector = self.model.get_sentence_vector(query)

        scores = np.dot(self.fasttext_matrix,question_vector.T)
        idx_scores = [(idx, score) for idx, score in enumerate(scores)]
        top_k_scores = OrderedDict(
            sorted(idx_scores, key=(lambda tup: tup[1]), reverse=True)[:top_k]
        )
        return top_k_scores

    def fit(self):
        self.df = pd.DataFrame.from_dict(self.paragraphs)
        self.df["text"] = self.df["text"].apply(lambda x: " ".join(x))
        self.fasttext_matrix = np.zeros((self.df.shape[0],self.model.get_dimension()))
        for i,t in enumerate(self.df["text"]):
            self.fasttext_matrix[i,:] = self.model.get_sentence_vector(self._preprocess_text(t))
